...
| CI recipes | Optional GitHub Actions recipe exists under `examples/ci/`; it calls the CLI and uploads `.harness/**` evidence without adding CI-only source-of-truth state. | Blocking or advisory checks for teams that want objective harness gates. | Example recipe available; no CI platform package or enforcement adapter is shipped. |
