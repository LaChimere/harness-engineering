...
The deterministic `harness` CLI includes schema validation, Harness doctor checks, declared local project health checks, verifier-only eval validation, deterministic stub agent runs, behavioral eval runs, trace validation/import, offline LLM-judge policy/result validation, limited-adapter scope validation, native execution-loop evidence validation, deterministic GC audit/validation, and a read-only native agent-facing assessment command. It consumes the local schemas and examples, but it still does not ship an installable host plugin package, CI adapter, skill adapter, or live model execution behavior.
...
`harness health` executes declared local project health checks from the optional `health.checks` block in `harness.yaml` and emits Markdown by default or schema-valid JSON with `--format json`. Health checks are separate from structural doctor checks. Because the current runner is declaration-gated process execution rather than a runtime sandbox, execution requires the explicit `--accept-unsandboxed-execution` flag after reviewing the configured commands. Each health check must declare a command, timeout, trust requirements, and optional artifact links; the command is refused unless trust is `sandboxed`, `sandbox_required` is `process`, network/secret/host-file access are false, and `allowed_outputs` includes the configured `health.output_dir`. Health also loads the existing approval and sandbox policy artifacts and records `sandbox_enforcement: declarative` and `runtime_enforced: false` so consumers do not confuse declaration gating with a runtime sandbox. `--output <path>` must stay under `health.output_dir` and rejects symlinked write targets. Passing checks exit `0`; failed checks exit `5`; schema, policy, trust, missing-artifact, and unsafe-declaration refusals exit `1`.

`harness gc audit` runs deterministic entropy checks and emits Markdown by default or schema-valid JSON with `--format json`. Current categories cover broken references, duplicate ids in supported harness-adjacent namespaces, stale schema-version ranges, and optional evidence-backed issues from verification, run-result, scoreboard, trace, and judge-result artifacts. Pass `--verification`, `--run-results`, `--scoreboard`, `--trace`, or `--judge-result` to include those artifacts in an audit; `--run-results` accepts one JSON object, one JSON array, or a JSONL/NDJSON ledger file. `--repair-actions-dir` and `--capability-ledger` select the stable-id namespaces scanned for duplicate ids. GC audit output conforms to `schemas/gc-evidence.schema.json`; each finding includes evidence refs, confidence, a proposed cleanup slice, blast radius, and atomicity notes. `--output <path>` writes inside the selected root and refuses to overwrite existing files, preserving audit history; pass a unique path such as `.harness/gc/<audit-id>.json` for each run, and use `--format json` for artifacts you intend to feed back into `harness gc validate`. GC audit is read-only and never applies cleanup.

`harness gc validate <artifact>` validates a GC evidence artifact against schema and semantic checks such as non-empty evidence refs, non-empty cleanup targets, unique cleanup slice ids, fragment-free cleanup target files, and resolvable local evidence/target paths. External URI and fragment-only references remain allowed as evidence and decision references, but cleanup targets and local previous-audit refs must resolve inside the selected root. Use `--skip-reference-checks` only when validating archived GC evidence outside the checkout where it was produced.
...
- `verification-evidence` is emitted only for schema-valid self-verification evidence whose spec reread is not matched, whose acceptance checks or recorded commands failed, or whose unresolved risks are non-empty.
- `execution-evidence` is emitted only for schema-valid run-result evidence whose top-level status is not `passed`; JSON objects, arrays, and JSONL ledgers are supported.
- `eval-evidence` is emitted only for schema-valid scoreboard evidence whose overall status is not `passed`. Expected negative-control failures inside a passing scoreboard are not reported as GC findings.
- `trace-evidence` is emitted only for schema-valid trace evidence with a non-zero exit code or action-level errors.
- `judge-calibration` is emitted only for schema-valid judge-result evidence whose calibration status is not `passed`; full policy digest, threshold, staleness, and blocking-eligibility checks remain owned by `harness report`.

GC can cite promotion and retirement decision references, but this CLI slice does not yet promote or retire durable rules, templates, or evals automatically. Future promotion should require repeated evidence rather than one-off preferences, behavioral promotion must include holdout evidence, and any retirement should cite evidence showing the rule, template, or eval no longer adds value.
...
`harness run <task>` executes a configured eval task through the deterministic stub runner. It validates the harness, runner, task, and model profile; enforces `credential_reference.source: stub`; requires explicit cost/token/request budgets; checks the runner's task and verifier binding; writes the recorded oracle or broken-twin output to `.harness/agent-outputs/`; runs the task verifier; and emits trace, verifier-result, and run-result artifacts. Verifier-result artifacts conform to `schemas/verifier-result.schema.json`. If the harness refuses the run before stub output emission, the trace records a system refusal with zero stub requests and no agent-output artifact. `--session-id` associates multiple runs with one continuity session; if omitted, the CLI reads `continuity.session_id_env` from the harness or generates a session id. `--case oracle|broken-twin` selects the expected verifier outcome, and `--run-id` must be a safe artifact id.

`harness run --external-candidate <path>` imports a repo-local candidate generated outside harness, copies it into `.harness/agent-outputs/`, runs the configured verifier, and emits `external-import` run-result and trace artifacts. This is the Copilot-as-model/agent-mediated smoke path: it validates model output against harness evidence without executing a model provider, reading live credentials, or charging model usage. External-import traces use `determinism_level: external-import`, `credential_reference.source: external`, zero external usage, and provenance links for the original candidate plus copied agent output; they are not counted as `eval` agent runs, scoreboards, or `assess` run-result evidence. With external imports, `--case oracle|broken-twin` declares the expected verifier outcome only; it does not select or mutate the supplied candidate.
...
The read/report commands (`adapter validate`, `assess`, `gc audit`, `gc validate`, `loop validate`, `validate`, `doctor`, `trace validate`, `verify`, `report`) and the no-op `migrate` command may point `--root` at another checkout for inspection. Unlike those read/report commands, `harness health`, `harness eval validate`, `harness run`, and `harness eval run` may execute declaration-gated local commands in the selected root, including when `--root` points at another checkout. User-provided scope, matrix, file, continuity, verification, task, spec, artifact, doctor-output, health-result, health-output, eval-output, trace-output, scoreboard-output, assessment input, repair-action, repair-actions-dir, gc-output, and migration-output paths are still constrained to the selected root; `--trusted-repair-action` accepts only a safe repair-action id. Doctor output, health output, GC output, eval output, verifier-result output, trace output, scoreboard output, migration output, and init writes also reject symlinked write targets.
...
For `harness health`, process exit status is computed from the top-level health status: `passed` exits `0`; command check failures exit `5`; unsafe declarations, policy mismatches, missing declared artifacts, schema errors, and other health evidence errors exit `1`.
