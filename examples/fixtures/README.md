...
The manifest includes JSON Schema valid/invalid fixtures and custom semantic checks for invariants that JSON Schema cannot express directly. Capability-matrix checks use shared manifest rules and stable error-code prefixes for evidence-id references, tier thresholds, out-of-scope surface boundaries, and target-selection rules. Adapter-scope checks cover selected-host alignment, capability subset claims, CLI management modes, evidence references, write-class limits, and non-authoritative local state. Execution-loop negative fixtures under `examples/fixtures/execution-loop/` are schema-valid; the manifest proves their shape is valid, then `harness loop validate` CLI tests reject them because the failure is semantic rather than structural.
