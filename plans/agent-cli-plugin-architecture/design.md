...
| `harness doctor --format json` | Yes | `schemas/doctor-result.schema.json` | Top-level `status`: `passed`, `failed`, `warning`; per-check `outcome`: `passed`, `failed`, `skipped`. | Top-level `schema_version`, `run_id`, `harness_version`; no `generated_at`. | No top-level `issues`; checks carry `severity`, `outcome`, `evidence`, `remediation`, fixtures, and trust metadata. | Add `generated_at`. Add `issues` only as a normalized summary derived from non-passing checks; do not remove detailed `checks`. |
| `harness health --format json` | Yes | `schemas/health-result.schema.json` | Top-level `status`: `passed`, `failed`, `error`; per-check `status`: `passed`, `failed`, `error`, `skipped`. | Top-level `schema_version`, `run_id`, `harness_version`, `source`, `sandbox_enforcement`, `runtime_enforced`; no `generated_at`. | No top-level `issues`; per-check failures use `failure_code`, `summary`, stdout/stderr fields, artifacts/evidence, and trust metadata. | Add `generated_at`. Add `issues` as a normalized top-level summary derived from failed/error/skipped checks; keep per-check details canonical for health-specific reasoning. Decide whether `--format json --output` should continue writing human stdout while the JSON contract lives in the output file. |
| `harness assess --format json` | Yes | `schemas/assessment.schema.json` | Top-level `status`: `ready`, `needs-work`, `missing-harness`; nested scorecard/routing statuses. | Top-level `schema_version`, `assessment_id`; nested `source.root`, `source.harness`, `source.cli_version`; no top-level `run_id`, `harness_version`, or `generated_at`. | No top-level `issues`; `missing_primitives` and `recommendations` describe product gaps, not command failures. | Add top-level `harness_version` and `generated_at`; rename `source.cli_version` to `source.harness_version` through the documented migration. Do not conflate recommendations with `issues`; add `issues` only for command/contract problems that block machine consumption. |
| `harness gc audit --format json` | Yes | `schemas/gc-evidence.schema.json` | No status field in the JSON artifact today; current output has `findings[]`. | Top-level `schema_version`, `audit_id`, `generated_at`; no `run_id` or `harness_version`. | `findings[]` are cleanup findings, not command failures; no top-level `issues`. | Add top-level `status` (`passed` or `findings`) and optional `issues` for audit/contract problems. Keep `findings` semantically separate from `issues`. Add optional `harness_version`; keep `audit_id` rather than inventing `run_id`. |
| `harness trace validate --format json` | Yes | Current command output is an ad-hoc validation result with `schema_version`, `status`, and `traces[]`; trace artifacts themselves use `schemas/trace.schema.json`. | Validation result `status`: `passed`, `failed`; per-trace `status`: `passed`, `failed`. Trace artifacts have `determinism_level`, not top-level status. | Validation result has only `schema_version`; trace artifacts have `run_id`, `harness_version`, timestamps, environment snapshot, usage, budgets. | Validation result uses `traces[].issues: string[]`; trace artifacts have action-local structured `errors[]`. | Add a dedicated schema for the validation result itself, then map validation issue strings to `ICliJsonIssue`. Do not treat `schemas/trace.schema.json` as the schema for the `trace validate` command output. |
