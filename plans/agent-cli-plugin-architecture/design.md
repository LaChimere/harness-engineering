...
|---|---|---|
| Schemas | `schemas/model-profile.schema.json`, `schemas/agent-runner.schema.json`, `schemas/runner-readiness.schema.json` | Delete entirely. No reusable contract pieces exist. |
| Trace/profile provenance schemas | `schemas/trace.schema.json`, `schemas/profile-run.schema.json` `environment_snapshot.runner` and `environment_snapshot.model_profile` field descriptions | Keep the fields, but update descriptions/comments so they are read-only imported provenance for whatever external agent/model produced the evidence, not Harness-configured execution. No schema version bump is required if field shapes remain unchanged; bump only if the field shape or vocabulary changes. |
| Harness config | `model_profiles`, `agent_runners` fields in `harness.yaml` examples/schema | Remove from default user substrate and from `schemas/harness.schema.json` required/properties; update `harness init` so it no longer emits them. |
| CLI commands | `harness runner` subtree, `harness run` as agent/model execution | Delete the `runner` subtree. Delete `harness run` entirely; the product does not need evidence-import/evidence-verification dedicated verbs today. Candidate evidence arrives through explicit `--external-candidate` flags or declarative runner placeholders if those features already exist. If a future need for batch-import surfaces, design it with its own slug. |
| Libraries | `src/lib/agent-runner.ts`, `src/lib/runner-readiness.ts`, and any `src/commands/runner*.ts` files | Delete `agent-runner.ts` and `runner-readiness.ts` entirely. No evidence-import/evidence-verification library surface is needed; host agents produce artifacts, and the existing `harness verify` or `harness eval` surfaces already handle verification. If modular evidence-import logic is needed in the future, design it in a dedicated slug. |
| Examples | model profile, agent runner, live-readiness, deterministic stub runner examples under `examples/` and `.harness/` | Delete. Do not keep "archived" variants of removed product concepts. |
| Docs | live-runner/provider-backed/stub-runner language in README, guides, AGENTS.md, planning slug | Remove from public product docs and replace with host-agent-produced evidence language. |
| Tests | runner/eval-run tests asserting Harness-owned model execution | Delete all tests for `agent-runner.ts`, `runner-readiness.ts`, and `harness runner` / `harness run` command surfaces. Retain only tests for `harness profile run` (evidence consumer, not executor) and `harness eval` (task/verifier execution). Before cleanup, inventory all tests that import from the deleted modules and update or delete them. |
...
| `harness doctor --format json` | Yes | `schemas/doctor-result.schema.json` | Top-level `status`: `passed`, `failed`, `warning`; per-check `outcome`: `passed`, `failed`, `skipped`. | Top-level `schema_version`, `run_id`, `harness_version`; no `generated_at`. | No top-level `issues`; checks carry `severity`, `outcome`, `evidence`, `remediation`, fixtures, and trust metadata. | Add `generated_at` if the migration plan classifies it as useful run provenance. Add `issues` only as a normalized summary derived from non-passing checks; do not remove detailed `checks`. |
| `harness health --format json` | Yes | `schemas/health-result.schema.json` | Top-level `status`: `passed`, `failed`, `error`; per-check `status`: `passed`, `failed`, `error`, `skipped`. | Top-level `schema_version`, `run_id`, `harness_version`, plus `source`; no `generated_at`. | No top-level `issues`; per-check failures use `failure_code`, `summary`, stdout/stderr fields, and artifacts/evidence. | Add `generated_at`. Add `issues` as a normalized top-level summary derived from failed/error/skipped checks; keep per-check details canonical for health-specific reasoning. |
| `harness assess --format json` | Yes | `schemas/assessment.schema.json` | Top-level `status`: `ready`, `needs-work`, `missing-harness`; nested scorecard/routing statuses. | Top-level `schema_version`, `assessment_id`; nested `source.root`, `source.harness`, `source.cli_version`; no top-level `run_id`, `harness_version`, or `generated_at`. | No top-level `issues`; `missing_primitives` and `recommendations` describe product gaps, not command failures. | Rename `source.cli_version` to `harness_version` only through the documented migration. Do not conflate recommendations with `issues`; add `issues` only for command/contract problems that block machine consumption. |
| `harness gc audit --format json` | Yes | `schemas/gc-evidence.schema.json` | No status field in the JSON artifact today; command internals distinguish `passed` vs `findings`. | Top-level `schema_version`, `audit_id`, `generated_at`; no `run_id` or `harness_version`. | `findings[]` are cleanup findings, not command failures; no top-level `issues`. | Add top-level `status` (`passed` or `findings`) and optional `issues` for audit/contract problems. Keep `findings` semantically separate from `issues`. Decide in the migration plan whether `harness_version` is needed; do not invent run identity unless the command actually needs one. |
| `harness trace validate --format json` | Yes | Current command output is an ad-hoc validation result with `schema_version`, `status`, and `traces[]`; trace artifacts themselves use `schemas/trace.schema.json`. | Validation result `status`: `passed`, `failed`; per-trace `status`: `passed`, `failed`. Trace artifacts have `determinism_level`, not top-level status. | Validation result has only `schema_version`; trace artifacts have `run_id`, `harness_version`, timestamps, environment snapshot, usage, budgets. | Validation result uses `traces[].issues: string[]`; trace artifacts have action-local structured `errors[]`. | Add or identify a schema for the validation result itself, then map validation issue strings to `ICliJsonIssue`. Do not treat `schemas/trace.schema.json` as the schema for the `trace validate` command output. |
| `harness profile run --format json` | Yes | `schemas/profile-run.schema.json` | No top-level `status` today; `handoff.status`: `met`, `not_met`, `inconclusive`; action summaries repeat the same vocabulary. | Top-level `schema_version`, `run_id`, `harness_version`, `generated_at`, profile refs, and `evidence_inputs`. | Required `errors: string[]`; no `issues`. | Add top-level `status` mirroring `handoff.status` for shared command parsing. Replace legacy command-result `errors[]` with structured `issues[]` in the same pre-release schema migration. |
