...
Evidence: Stage 12 ships `harness assess` as the native agent-facing adapter path instead of a skill-first adapter. The command is read-only, validates its output with `schemas/assessment.schema.json`, reads harness/doctor/eval-plan/run-result/trace/scoreboard/report/repair-action artifacts, emits maturity and missing-primitive guidance, and routes implementation requests to applicable repair actions only when a trusted approval id is supplied, native execution-loop guidance, or a CLI fallback while marking external workflow skills as unavailable source material rather than implementation routes. Review refinements ensure schema-invalid, duplicate-id, untrusted, unapproved, and non-applicable repair actions are surfaced but not selected, repair-action `equivalent_cli_command` values are not emitted as executable assessment route commands, Stage 12 implementation does not select external-source-material routes, and `artifacts_read` includes composed harness references validated from `harness.yaml`. The CLI path and non-skill trigger/eval decision are documented in `docs/guides/cli.md`; routing is illustrated by a generated `harness assess` output at `examples/assessments/repair-action-routing.json`. A later adapter follow-up adds canonical shared skill source files under `skills/<id>/` as CLI/schema consumers only; they do not replace `harness assess` or define skill-only product rules. Claude Code, Codex, Copilot CLI, and Gemini CLI packaging now validate local adapter layout, skill parity, hook safety, and plugin manifests without claiming public marketplace/global installability; Codex and Copilot CLI have explicit live/model-backed `harness-doctor` invocation smoke evidence, and Copilot CLI has real-temp-project diagnosis/repair/explicit-health/negative-health evidence. Public docs now describe repo-local adapter packages directly while keeping unverified public marketplace/global installability out of scope. Comprehensive code/docs/tests/security/types/simplify review plus rubber-duck converged with no material comments after tightening adapter prelude, path/symlink, marketplace, manifest, and health-guidance checks. Remaining unavailable-host and unverified-host evidence is tracked in the adapter slug. Verified with `bun run check`, `bun run test:unit`, `bun run test:e2e`, adapter checks, PR 8 stale wording searches with approved exceptions recorded in the adapter slug, schema fixture validation, and `git diff --check`.
