...
`recurring-profile.schema.json` defines deterministic maintenance profile contracts with structured evidence inputs, trigger thresholds, stop conditions, allowed actions, handoff metadata, and trust requirements. Recurring profiles are single-run CLI consumers of substrate artifacts; scheduling and persistence are external. `profile-run.schema.json` records one profile execution with hashed evidence inputs, top-level status, trigger and stop-condition observations, one deterministic summary action, optional previous-run continuity, structured issues for non-met or inconclusive outcomes, and a handoff decision. The GC stability profile consumes GC and health evidence; it does not run cleanup, mutate the capability ledger, call models, or create prompt-only profile state.
...
`gc-evidence.schema.json` uses a closed category set plus required top-level `harness_version` and status. `status: passed` means no findings were emitted; `status: findings` means `findings[]` contains reviewable cleanup proposals. Current categories cover mechanical harness structure (`broken-reference`, `duplicate-id`, `stale-schema-version`) and evidence-backed validation gaps (`verification-evidence`, `execution-evidence`, `eval-evidence`, `trace-evidence`, `judge-calibration`). New GC categories should be added through a schema-versioned change with fixtures, algorithms, and false-positive policy.

`trace-validate-result.schema.json` is the command-result schema for `harness trace validate --format json`. It is intentionally separate from `trace.schema.json`: trace artifacts remain the evidence records, while trace validation results report per-trace structured issues and an aggregate top-level failed-trace summary.
